...
void Controller::controlCartes()
{
    while(1){
        ...
        std::vector<Carte*> const cartes = detection->getCartes();
...
void Controller::controlPersonnage(std::vector<Carte*> cartes, unsigned int loop_begin, unsigned int marqueur)
{
    for(unsigned int i = loop_begin; i<cartes.size();i++){
        ...
        if(cartes.at(i)->getId() == 9){
            ...
            if(cartes.at(i)->getArgumentId()==11){
                controlPersonnage(cartes, loop_begin+1,2);
                controlPersonnage(cartes, loop_begin+1,2);
...
void Controller::displayFunctions(std::vector<Carte*> cartes)
{
    ...
    {
        QString const string = cartes.at(i)->getName();
